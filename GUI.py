import tkinter as tk from tkinter import scrolledtext, messagebox import sqlite3 import requests import cv2 import numpy as np from PIL import Image, 
ImageTk
...
def reparameterize_curve(points, num_points=75):
...
def process_frame(frame):
    ...
    lower_exclude = np.array([0, 0, 0])  # Lower bound for black
    upper_exclude = np.array([179, 255, 30]) # Upper bound for black
    black_mask = cv2.inRange(hsv_image, lower_exclude, upper_exclude)
    inverted_mask = cv2.bitwise_not(black_mask)
    hsv_result = cv2.bitwise_and(frame, frame, mask=inverted_mask)
...
pl1019492@7118L05 25-26_PWP % cat p_line_detection_3.py   
import tkinter as tk from tkinter import scrolledtext, messagebox import sqlite3 import requests import cv2 import numpy as np from PIL import Image, 
ImageTk
